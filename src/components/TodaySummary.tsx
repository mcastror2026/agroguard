    ...
    return (
      ...
      >
        ...
        <div className="grid md:grid-cols-2 gap-3 md:gap-4">
          ...
          {obs && (
            <div className="bg-white border border-gray-200/50 rounded-2xl p-5">
              <div className="text-xs font-light text-gray-500 mb-3 uppercase tracking-wider">Estación Local</div>
              <div className="text-2xl font-light text-gray-900 tracking-tight">
                {fmt(obs?.temperature, 1)}°C · {fmt(obs?.humidity, 0)}%
              </div>
              {obs?.station && <div className="text-xs text-gray-400 mt-2 font-light">{obs.station}</div>}
            </div>
          )}
        </div>
      </motion.section>
